...
from modulestore.django import modulestore, clear_existing_modulestores, SignalHandler
from modulestore.tests.mongo_connection import MONGO_PORT_NUM, MONGO_HOST
from modulestore.tests.factories import XMODULE_FACTORY_LOCK
...
def mixed_store_config(data_dir, mappings, store_order=None):
    ...
    store = {
        'default': {
            'ENGINE': 'modulestore.mixed.MixedModuleStore',
            'OPTIONS': {
                'mappings': mappings,
...
@patch('modulestore.django.create_modulestore_instance', autospec=True)
...
class SharedModuleStoreTestCase(ModuleStoreIsolationMixin, CacheIsolationTestCase):
    ...
    How to use::
        ...
        from modulestore.tests.django_utils import SharedModuleStoreTestCase
