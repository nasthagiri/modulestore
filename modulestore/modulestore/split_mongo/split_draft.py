...
from modulestore.split_mongo.split import SplitMongoModuleStore, EXCLUDE_ALL
from xmodule.exceptions import InvalidVersionError
from modulestore import ModuleStoreEnum
from modulestore.exceptions import InsufficientSpecificationError, ItemNotFoundError
from modulestore.draft_and_published import (
...
from modulestore.split_mongo import BlockKey
...
class DraftVersioningModuleStore(SplitMongoModuleStore, ModuleStoreDraftAndPublished):
    ...
    def clone_course(self, source_course_id, dest_course_id, user_id, fields=None, revision=None, **kwargs):
        """
        See :py:meth: modulestore.split_mongo.split.SplitMongoModuleStore.clone_course
        """
    ...
    def get_course_history_info(self, course_locator):
        """
        See :py:meth `modulestore.split_mongo.split.SplitMongoModuleStore.get_course_history_info`
        """
    ...
    def get_course_successors(self, course_locator, version_history_depth=1):
        """
        See :py:meth `modulestore.split_mongo.split.SplitMongoModuleStore.get_course_successors`
        """
        course_locator = self._map_revision_to_branch(course_locator)
    ...
    def get_block_generations(self, block_locator):
        """
        See :py:meth `modulestore.split_mongo.split.SplitMongoModuleStore.get_block_generations`
        """
